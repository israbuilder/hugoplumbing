...
class GoogleAdsAuthService
{
    ...
    public function validateState(?string $state): void
    {
        ...
        if (
            ...
            throw new RuntimeException(
                'Invalid Google Ads OAuth state.'
            );
        }
    }
    ...
    public function exchangeCode(
        ...
        try {
            ...
            throw new RuntimeException(
                'Google Ads OAuth exchange failed: '
                . $e->response?->body(),
                previous: $e
            );
        }
        ...
        return DB::transaction(function () use ($tokenData) {
            ...
            if (!$account) {
                $account = IntegrationAccount::create([
                    ...
                    'external_account_id' => 'google_ads',
                    'name' => 'Google Ads',
                    'status' => 'connected',
                    'connected_at' => now(),
                    'metadata' => [
    ...
    public function getValidAccessToken(
        ...
        if (!$account->token) {
            throw new RuntimeException(
                'Google Ads token missing.'
            );
        }

        if (!$account->token->expiresSoon()) {
